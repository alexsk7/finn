...
- **`db.py`** — `get_conn()` opens SQLite with WAL mode and `row_factory=sqlite3.Row`. `init_db()` creates all tables idempotently. DB file lives at project root as `finance.db`. Idempotent column migrations are applied after `executescript` in a try/except block.

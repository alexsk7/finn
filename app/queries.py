...
def get_dashboard_summary() -> dict:
    with get_conn() as conn:
        ...
        re_rows = conn.execute(
            "SELECT estimated_value, mortgage_balance, account_id FROM real_estate"
        ).fetchall()
...
def get_real_estate() -> list[dict]:
    with get_conn() as conn:
        ...
        loan_accounts = {
            r['id']: r['name']
            for r in conn.execute(
                "SELECT id, name FROM accounts WHERE type='loan' AND is_active=1"
            ).fetchall()
        }
        balances = _compute_balances(conn)
    out = []
...
def get_rebalance(new_cash: float = 0.0) -> dict:
    ...
    for cls in all_classes:
        ...
        preferred = (
            next((h for h in class_holdings if h["account_type"] == "brokerage"), None)
            or (class_holdings[0] if class_holdings else None)
        )
        seen = set()
        existing = []
        for h in class_holdings:

...
def delete_account(account_id: int) -> dict:
    with get_conn() as conn:
        ...
        if txn_count > 0:
            return {
                "ok": False,
                "error": f"Account has {txn_count} transaction(s). Delete them first.",
            }
...
def import_holdings_csv(csv_text: str, account_id: int) -> dict:
    ...
    COL_ALIASES = {
        ...
        "average_cost": "cost_per_share",
        "unit_cost": "cost_per_share",
    ...
    CLASS_MAP = {
        ...
        "fund": "us_equity",
        "exchange-traded fund": "us_equity",
        "exchange traded fund": "us_equity",
        "bond": "bond",
        "fixed income": "bond",
        "fixed_income": "bond",
        "cash & money market": "cash_equiv",
        "cash and money market": "cash_equiv",
        "money market": "cash_equiv",
        "crypto": "crypto",
        "cryptocurrency": "crypto",
        "reit": "real_estate_fund",
...
def upsert_property_cost(property_id: int, cost_year: int, cost_month: int, amount: float, memo: str | None) -> dict:
    with get_conn() as conn:
        ...
        """,
            (property_id, cost_year, cost_month),
        ).fetchone()
    return dict(row)
